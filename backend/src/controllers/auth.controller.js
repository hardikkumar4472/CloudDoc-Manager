...
export const requestPasswordChange = async (req, res) => {
  try {
    ...
    res.json({ message: "OTP sent to your email to confirm password change" });
  } catch (err) {
...
export const setVaultPin = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Server error setting PIN", error: err.message });
  }
};
...
export const verifyVaultPin = async (req, res) => {
  try {
    ...
    if (!user.vaultPin) return res.status(400).json({ message: "No PIN set" });
    ...
    res.status(500).json({ message: "Server error verifying PIN", error: err.message });
  }
};
